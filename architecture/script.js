/* ФОРМА — архитектурное бюро · interactions */
(function () {
  "use strict";
  ...
  if (form) {
    ...
    [name, phone].forEach((i) => i.addEventListener("input", () => {
      if (i.classList.contains("invalid")) { i.classList.remove("invalid"); field(i).classList.remove("show-err"); }
    }));
  }
  ...
  $$('a[href^="#"]').forEach((a) => a.addEventListener("click", (e) => {
    ...
    if (!t) return;
    e.preventDefault();
    const y = t.getBoundingClientRect().top + window.scrollY - (nav.offsetHeight + 12);
